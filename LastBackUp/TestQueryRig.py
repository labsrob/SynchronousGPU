...
def rolling_mean_std(data1, window):
    ...
    return mean_dict[col], std_dict[col]
...
rolled_m, rolled_s = rolling_mean_std(df_interp, s_fetch)

col = df_interp.columns[2]
col_data = df_interp[col]
print('TP00', col_data)
df_x = pd.DataFrame(rolled_m, columns=[col], index=df_interp.index)
df_s = pd.DataFrame(rolled_s, columns=[col], index=df_interp.index)

self.im10.set_xdata(np.arange(batch_TT))
self.im10.set_ydata(df_x)