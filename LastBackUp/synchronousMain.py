...
#                 np = cp
#                 GPU_ENABLED = False #True
#                 print(f"Using Nvidia-GPU for Accelerated Computations [CuPy]")
#                 if gpu_info:
...
def tabbed_cascadeMode(pWON, cType):
    ...
    if uCalling == 1:
        pMode = 'Auto Triggered Live Process'
    elif uCalling == 2:
        pMode = 'User Triggered Live Process'
    elif uCalling == 3:
        pMode = 'User Triggered Post-Production'
    else:
...
class rollerPreTabb(ttk.Frame):
    ...
    def dataControlRP(self):
        ...
        if uCalling == 2 or uCalling == 3 and UseSQL_DBS:
            if self.running:
...
class tapeTempTabb(ttk.Frame):  # -- Defines the tabbed region for QA param - Tape Temperature --[]
            ...
            if UseSQL_DBS or UsePLC_DBS and self.running:
                ...
                if uCalling == 1:
                    ...
                    if keyboard.is_pressed("Alt+Q"):
                        ...
                        if len(self.ttD1) < s_fetch:
                            print(f"Warning: Data only has {len(self.ttD1)} rows, but rolling window is {s_fetch}.")
    ...
    def ttDataPlot(self):
        ...
        def rolling_mean_std(data1, window):
            ...
            for col in data1.columns:
                col_data = cp.asarray(data1[col].values)

                if len(col_data) < window:
                    # Not enough data for rolling window
                    mean_col = cp.full_like(col_data, cp.nan)
                    std_col = cp.full_like(col_data, cp.nan)
                else:
                    # Prepend NaNs to match original size after rolling
                    nan_pad = cp.full((window - 1,), cp.nan)
                    # Use rolling with convolution (efficient on GPU)
                    kernel = cp.ones(window) / window
                    mean_valid = cp.convolve(col_data, kernel, mode='valid')
                    mean_col = cp.concatenate([nan_pad, mean_valid])

                    # Rolling std: use squared diff from rolling mean
                    std_valid = cp.sqrt(
                        cp.convolve(cp.square(col_data), cp.ones(window)/window, mode='valid') -
                        cp.square(mean_valid)
                    )
                    std_col = cp.concatenate([nan_pad, std_valid])

                # Store in dict
                # mean_dict[col] = cp.asnumpy(mean_col).reshape(-1, 1)
                # std_dict[col] = cp.asnumpy(std_col).reshape(-1, 1)
                mean_dict[col] = cp.asnumpy(mean_col).ravel()
                std_dict[col] = cp.asnumpy(std_col).ravel()

            return mean_dict, std_dict

        # Call data loader Method-----------------------#
        if uCalling == 1 and UsePLC_DBS and self.running:
            ...
            if GPU_ENABLED:
                ...
                # Convert to cuPy Matrix and keep column Names --------------------------[3]
                cols = df1_interp.select_dtypes(include='number').columns
                df1 = cp.asarray(df1_interp[cols].to_numpy())
                df1_np = cp.asnumpy(df1)

                g1 = qtt.validCols(self.T1)
                df1 = pd.DataFrame(df1_np, columns=g1)
                TT = tt.loadProcesValues(df1)
            else:
                TT = tt.loadProcesValues(df1_interp)

        elif uCalling == 2 or uCalling == 3 and UseSQL_DBS and self.running:
            import VarSQL_TT as tt                                      # load SQL variables column names | rfVarSQL
            g1 = qtt.validCols(self.T1)
            df1 = pd.DataFrame(self.ttD1, columns=g1)
            df_c = df1.select_dtypes(include=["number"])
            df_interp = df_c.interpolate(method="linear", axis=0).ffill().bfill()

            TA = tt.loadPVT1(df_interp)
        ...
        #         cupy_data = cp.asarray(data2.values)
        #         rolled = rolling_window(cupy_data, 30)
        #         rolling_mean = cp.mean(rolled, axis=1)
        #         rolling_std = cp.std(rolled, axis=1)
        #
        #         # Back to pandas & Pad the beginning with NaNs to match original shape
        #         nan_pad = cp.full((30 - 1, rolling_mean.shape[1]), cp.nan)
        #
        #         mean_full = cp.vstack([nan_pad, rolling_mean])
        #         std_full = cp.vstack([nan_pad, rolling_std])
        #
        #         df_1 = pd.DataFrame(cp.asnumpy(mean_full), columns=data2.columns)
        #         df_2 = pd.DataFrame(cp.asnumpy(std_full), columns=data2.columns)
        #         # print('TP03', df_1.tail(10))
        #         # print('TP04', df_2.tail(10))
        #         print('TP04', df_1.index)
        #         series2 = df_1[data2.columns[2]].dropna()
        #         # Drop NaNs
        #         # TB = series.dropna()
        #         print('TP05b', series2.values[-1:][0])
        #
        #     else:
        #         print('GPU is not enabled....')
        #         TB = tt.loadPVT2(data2)
        #         print('T03', TB[2])
        #         # --------------------------------------------[]
        #         # print("Memory Usage:", p_data.info(verbose=False))
        ...
        if self.running:
            ...
            if UsePLC_DBS:
                if GPU_ENABLED:
                    ...
                    self.im10.set_ydata(pd.Series(TT[:, 1]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 1
                    self.im11.set_ydata(pd.Series(TT[:, 2]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 2
                    self.im12.set_ydata(pd.Series(TT[:, 3]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 3
                    self.im13.set_ydata(pd.Series(TT[:, 4]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 4
                    # S Plot Y-Axis data points for StdDev ----------------------------------------[S Plot]
                    self.im14.set_ydata(pd.Series(TT[:, 1]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im15.set_ydata(pd.Series(TT[:, 2]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im16.set_ydata(pd.Series(TT[:, 3]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im17.set_ydata(pd.Series(TT[:, 4]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])

                    # X Bar Plot for Ring 2 --------------------------------------------------------[ Ring 2 TT]
                    self.im18.set_ydata(pd.Series(TT[:, 5]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 1
                    self.im19.set_ydata(pd.Series(TT[:, 6]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 2
                    self.im20.set_ydata(pd.Series(TT[:, 7]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 3
                    self.im21.set_ydata(pd.Series(TT[:, 8]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 4
                    # S Plot for Ring 2 -------------------------------------------------------------[S Plot]
                    self.im22.set_ydata(pd.Series(TT[:, 5]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im23.set_ydata(pd.Series(TT[:, 6]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im24.set_ydata(pd.Series(TT[:, 7]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im25.set_ydata(pd.Series(TT[:, 8]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])

                    # X Bar Plot for Ring 3 --------------------------------------------------------[ Ring 3 TT]
                    self.im26.set_ydata(pd.Series(TT[:, 9]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 1
                    self.im27.set_ydata(pd.Series(TT[:, 10]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 2
                    self.im28.set_ydata(pd.Series(TT[:, 11]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 3
                    self.im29.set_ydata(pd.Series(TT[:, 12]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 4
                    # ------ Evaluate Pp for Ring 3 ---------#
                    self.im30.set_ydata(pd.Series(TT[:, 9]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im31.set_ydata(pd.Series(TT[:, 10]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im32.set_ydata(pd.Series(TT[:, 11]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im33.set_ydata(pd.Series(TT[:, 12]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])

                    # X Bar Plot for Ring 4 --------------------------------------------------------[ Ring 4 TT]
                    self.im34.set_ydata(pd.Series(TT[:, 13]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 1
                    self.im35.set_ydata(pd.Series(TT[:, 14]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 2
                    self.im36.set_ydata(pd.Series(TT[:, 15]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 3
                    self.im37.set_ydata(pd.Series(TT[:, 16]).rolling(window=s_fetch, min_periods=1).mean()[0:batch_TT])  # head 4
                    # ------ Evaluate Pp for Ring 4 ---------#
                    self.im38.set_ydata(pd.Series(TT[:, 13]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im39.set_ydata(pd.Series(TT[:, 14]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im40.set_ydata(pd.Series(TT[:, 15]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                    self.im41.set_ydata(pd.Series(TT[:, 16]).rolling(window=s_fetch, min_periods=1).std()[0:batch_TT])
                else:
                    # X Plot Y-Axis data points for XBar --------------------------------------------[Ring 1 TT]
            ...
            elif GPU_ENABLED and UseSQL_DBS:
                print('SET: GPU is enabled...') # last_row
                # X Plot Y-Axis data points for XBar --------------------------------------------[Ring 1 TT]
                # self.im10.set_ydata(last_row1)  # head 1

                self.im10.set_ydata(ydata)  # head 1
                # self.im11.set_ydata(df_1[data1.columns[3]].values[-1:][0][0:batch_TT])  # head 2
                # self.im12.set_ydata(df_1[data1.columns[4]].values[-1:][0][0:batch_TT])  # head 3
                # self.im13.set_ydata(df_1[data1.columns[5]].values[-1:][0][0:batch_TT])  # head 4
                # # S Plot Y-Axis data points for StdDev ----------------------------------------[S Plot]
                # self.im14.set_ydata(df_2[data1.columns[2]].values[-1:][0][0:batch_TT])
                # self.im15.set_ydata(df_2[data1.columns[3]].values[-1:][0][0:batch_TT])
                # self.im16.set_ydata(df_2[data1.columns[4]].values[-1:][0][0:batch_TT])
                # self.im17.set_ydata(df_2[data1.columns[5]].values[-1:][0][0:batch_TT])
                #
                # # X Bar Plot for Ring 2 --------------------------------------------------------[ Ring 2 TT]
                # self.im18.set_ydata(df_1[data1.columns[6]].values[-1:][0][0:batch_TT])  # head 1
                # self.im19.set_ydata(df_1[data1.columns[7]].values[-1:][0][0:batch_TT])  # head 2
                # self.im20.set_ydata(df_1[data1.columns[8]].values[-1:][0][0:batch_TT])  # head 3
                # self.im21.set_ydata(df_1[data1.columns[9]].values[-1:][0][0:batch_TT])  # head 4
                # # S Plot for Ring 2 -------------------------------------------------------------[S Plot]
                # self.im22.set_ydata(df_2[data1.columns[6]].values[-1:][0][0:batch_TT])
                # self.im23.set_ydata(df_2[data1.columns[7]].values[-1:][0][0:batch_TT])
                # self.im24.set_ydata(df_2[data1.columns[8]].values[-1:][0][0:batch_TT])
                # self.im25.set_ydata(df_2[data1.columns[9]].values[-1:][0][0:batch_TT])
                #
                # # X Bar Plot for Ring 3 --------------------------------------------------------[ Ring 3 TT]
                # self.im26.set_ydata(df_1[data2.columns[2]].values[-1:][0][0:batch_TT])  # head 1
                # self.im27.set_ydata(df_1[data2.columns[3]].values[-1:][0][0:batch_TT])  # head 2
                # self.im28.set_ydata(df_1[data2.columns[4]].values[-1:][0][0:batch_TT])  # head 3
                # self.im29.set_ydata(df_1[data2.columns[5]].values[-1:][0][0:batch_TT])  # head 4
                # # ------ Evaluate Pp for Ring 3 ---------# Std
                # self.im30.set_ydata(df_2[data2.columns[2]].values[-1:][0][0:batch_TT])
                # self.im31.set_ydata(df_2[data2.columns[3]].values[-1:][0][0:batch_TT])
                # self.im32.set_ydata(df_2[data2.columns[4]].values[-1:][0][0:batch_TT])
                # self.im33.set_ydata(df_2[data2.columns[5]].values[-1:][0][0:batch_TT])
                #
                # # X Bar Plot for Ring 4 --------------------------------------------------------[ Ring 4 TT]
                # self.im34.set_ydata(df_1[data2.columns[6]].values[-1:][0][0:batch_TT])  # head 1
                # self.im35.set_ydata(df_1[data2.columns[7]].values[-1:][0][0:batch_TT])  # head 2
                # self.im36.set_ydata(df_1[data2.columns[8]].values[-1:][0][0:batch_TT])  # head 3
                # self.im37.set_ydata(df_1[data2.columns[9]].values[-1:][0][0:batch_TT])  # head 4
                # # ------ Evaluate Pp for Ring 4 ---------#
                # self.im38.set_ydata(df_2[data2.columns[6]].values[-1:][0][0:batch_TT])
                # self.im39.set_ydata(df_2[data2.columns[7]].values[-1:][0][0:batch_TT])
                # self.im40.set_ydata(df_2[data2.columns[8]].values[-1:][0][0:batch_TT])
                # self.im41.set_ydata(df_2[data2.columns[9]].values[-1:][0][0:batch_TT])

            else:
                print('SET: GPU Not Enabled ....')
                # X Plot Y-Axis data points for XBar --------------------------------------------[Ring 1 TT]
