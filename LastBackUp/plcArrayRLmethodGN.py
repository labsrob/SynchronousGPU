...
arrayGN, GN_list, Idx1, y_common, array2D = [], [], [], [], []
db_number, start_offset, bit_offset = 211, 0, 0
...
def plcExec(db_number, nGZ, grp_step, fetch_no):
	...
	if group_step == 1:
		...
		print('\nSINGLE STEP SLIDE')
		print('=================')
		print('Array length:', len(arrayGN), 'Fetch Value', fetch_no)
		if len(arrayGN) == nGZ and fetch_no > 0:
			print('TP A')
			n2fetch = fetch_no
		elif len(arrayGN) == nGZ and fetch_no < 1:
			print('TP B')
			n2fetch = 1
		elif len(arrayGN) >= nGZ and fetch_no > 0:
			print('TP C')
			n2fetch = (len(arrayGN) + fetch_no)
		elif len(arrayGN) >= nGZ and fetch_no < 1:
			print('TP D')
			n2fetch = 0
		else:
		...
		print('\nSAMPLE SIZE SLIDE')
		print('=================')
		if fetch_no != 0 and len(arrayGN) >= nGZ:
	...
	while True:
		try:
			...
			cLayer = readReal(db_number, start_offset[0], bit_offset[0])  # Ring1 Position
			GN_list.append(cLayer)
			LineT1 = readReal(db_number, start_offset[256], bit_offset[0])
			GN_list.append(LineT1)
			RP1 = readReal(db_number, start_offset[260], bit_offset[0])
			GN_list.append(RP1)
			RP2 = readReal(db_number, start_offset[264], bit_offset[0])
			GN_list.append(RP2)
			RP3 = readReal(db_number, start_offset[268], bit_offset[0])
			GN_list.append(RP3)
			RP4 = readReal(db_number, start_offset[272], bit_offset[0])
			GN_list.append(RP4)
			RP5 = readReal(db_number, start_offset[276], bit_offset[0])  # Pipe Direction
			GN_list.append(RP5)
			RP6 = readReal(db_number, start_offset[280], bit_offset[0])  # Current Layer
			GN_list.append(RP6)
			RP7 = readReal(db_number, start_offset[284], bit_offset[0])  # Pipe Direction
			GN_list.append(RP7)
			RP8 = readReal(db_number, start_offset[288], bit_offset[0])  # Current Layer
			GN_list.append(RP8)
			RP9 = readReal(db_number, start_offset[292], bit_offset[0])  # Current Layer
			GN_list.append(RP9)
			RP10 = readReal(db_number, start_offset[294], bit_offset[0])  # Current Layer
			GN_list.append(RP10)

			# Deposit list column content into rows array ----------[]
			if len(GN_list) > 12:
				del GN_list[0:(len(GN_list) - 12)]				# trim columns to shape
				print('\nResetting Column Size...', len(GN_list))

			arrayGN.append(GN_list)
			print('\nLIST COLUMN:', len(GN_list))
			print('LIST D_ROWS:', len(arrayGN))
			print('Next Break @:', (n2fetch + nGZ) - len(arrayGN))

			if group_step == 1:
				# Beautiful Purgatory Procedure ------------------------------[TODO - CODEFY SAMPLE SIZE]
				if len(arrayGN) >= n2fetch and fetch_no <= 30:		# Sample Size
					del arrayGN[0:(len(arrayGN) - nGZ)]  			# [static window]
					print('Resetting Rows on Static.', len(arrayGN))
					break
				elif len(arrayGN) >= 31 and (fetch_no + 1) >= 31:
					del arrayGN[0:(len(arrayGN) - fetch_no)]  		# [moving window]
					print('Resetting Rows on Move..', len(arrayGN))
					break
				else:
					pass
			else:
				if group_step > 1 and len(arrayGN) == n2fetch and fetch_no <= 30:
					print('Keeping No of Rows on Static... Array Size:', len(arrayGN))
					break
				if group_step > 1 and len(arrayGN) >= (nGZ + n2fetch) and fetch_no <= 31:
					del arrayGN[0:(len(arrayGN) - nGZ)]
					print('Resetting Rows on Static... Array Size:', len(arrayGN))
					break
				elif group_step > 1 and len(arrayGN) >= (nGZ + n2fetch) and fetch_no >= 31:
					del arrayGN[0:(len(arrayGN) - fetch_no)]
					print('Resetting Rows on Move.... Array Size:', len(arrayGN))
					break
				print('Breaking at..', (n2fetch + nGZ), ' Array Length:', len(arrayGN))
				# break
			GN_list.clear()  							# Clear content of the list for new round trip
