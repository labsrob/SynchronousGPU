...
class PanZoomPlot:
    def __init__(self, master):
        self.master = master
        # self.master.title("Pan & Zoom Plot")
        #
        # # Create Matplotlib Figure
        # self.fig = Figure(figsize=(6, 4))
        # self.ax = self.fig.add_subplot(111)
        # self.ax.plot(range(100), [i ** 0.5 for i in range(100)], label="y = sqrt(x)")
        # self.ax.legend()
        # self.ax.set_title("Use Mouse: Scroll = Zoom | Left-Drag = Pan")
        #
        # # Embed in Tkinter
    ...
    def on_press(self, event):
        ...
        if event.inaxes != self.ax or event.button != 1:  # left mouse
            return
        self.press_event = (event.xdata, event.ydata, self.ax.get_xlim(), self.ax.get_ylim())
