...
db_number, start_offset, bit_offset = 234, 0, 0		# SPC_LogQA_RP[DB234]
...
def plcExec(db_number, nGZ, grp_step, fetch_no):
	...
	while True:
		try:
			...
			# Tape Temperature ---R1
			STA = readInteger(db_number, start_offset[0], bit_offset[0])  		# Current Layer
			ST_list.append(STA)
			...
			ST_list.append(ST4)
			# R2 ------
			ST5 = readInteger(db_number, start_offset[10], bit_offset[0])  		# R2H1
			ST_list.append(ST5)
			ST6 = readInteger(db_number, start_offset[12], bit_offset[0])  		# R2H2
			ST_list.append(ST6)
			ST7 = readInteger(db_number, start_offset[14], bit_offset[0])  		# R2H3
			ST_list.append(ST7)
			ST8 = readInteger(db_number, start_offset[16], bit_offset[0])  		# R2H4
			ST_list.append(ST8)
			# R3 ------
			ST9 = readInteger(db_number, start_offset[18], bit_offset[0])  		# R3H1
			ST_list.append(ST9)
			ST10 = readInteger(db_number, start_offset[20], bit_offset[0])  	# R3H2
			ST_list.append(ST10)
			ST11 = readInteger(db_number, start_offset[22], bit_offset[0])  	# R3H3
			ST_list.append(ST11)
			ST12 = readInteger(db_number, start_offset[24], bit_offset[0])  	# R3H4
			ST_list.append(ST12)
			# R4 ------
			ST13 = readInteger(db_number, start_offset[26], bit_offset[0])  	# R4H1
			ST_list.append(ST13)
			ST14 = readInteger(db_number, start_offset[28], bit_offset[0])  	# R4H2
			ST_list.append(ST14)
			ST15 = readInteger(db_number, start_offset[30], bit_offset[0])  	# R4H3
			ST_list.append(ST15)
			ST16 = readInteger(db_number, start_offset[32], bit_offset[0])  	# R4H4
			ST_list.append(ST16)

			# Deposit list column content into rows array ----------[]
			if len(ST_list) > 17:
				del ST_list[0:(len(ST_list) - 17)]				# trim columns to shape
				print('\nReseSTing Column Size...', len(ST_list))
			...
			if group_step == 1:
				...
				if len(arrayST) >= n2fetch and fetch_no <= 30:		# sample size = 30
					del arrayST[0:(len(arrayST) - nGZ)]  			# [static window]
					print('ReseSTing Rows on Static.', len(arrayST))
					break
				elif len(arrayST) >= 31 and (fetch_no + 1) >= 31:
					del arrayST[0:(len(arrayST) - fetch_no)]  		# [moving window]
					print('ReseSTing Rows on Move..', len(arrayST))
					break
				else:
					pass
			else:
				...
				if group_step > 1 and len(arrayST) >= (nGZ + n2fetch) and fetch_no <= 31:
					...
					print('ReseSTing Rows on Static... Array Size:', len(arrayST))
					break
				elif group_step > 1 and len(arrayST) >= (nGZ + n2fetch) and fetch_no >= 31:
					del arrayST[0:(len(arrayST) - fetch_no)]
					print('ReseSTing Rows on Move.... Array Size:', len(arrayST))
					break
