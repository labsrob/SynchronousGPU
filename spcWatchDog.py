...
def watchDogController():
	...
	sqlrdy = plc.readBool(db_number, s_offset[1], b_offset[5])			# SQL table is ready
	msctcp = plc.readInteger(db_number, s_offset[2], b_offset[0])		# Machine State Code (msc)
	in_Use = plc.readBool(db_number, s_offset[3], b_offset[0])			# SPC is engaged
	m2mACK = plc.readBool(db_number, s_offset[15], b_offset[0])			# High bit to allow TCP lay tape
	sqlACK = plc.readBool(db_number, s_offset[15], b_offset[1])			# High Bit to assure SQL connectivity
	time.sleep(10)														# 10 seconds non-blocking interval
	rt_satus = dict(zip(machineCode_Data, codeDescript))
	msc_rt = rt_satus[msctcp]
	print('\nTCP01 Status:', msc_rt)

	return sysRun, sysidl, sysrdy, sqlrdy, msctcp, in_Use, m2mACK, sqlACK
...
def watchDog(listener, splash):
	...
	if listener.is_alive():
		...
		listener.stop()
		splash.destroy()
		print('Listener service, no longer available...')
	...
	while True:
		try:
			...
			sysRun, sysidle, sysrdy, sqlrdy, msctcp, spcInUse, m2mACK, sqlACK = watchDogController()
			# -----------------------------------------------------[]
		except Exception as err:
			print(f"Exception Error: '{err}'")
			rtError = True
			...
			elif not sysrdy and not spcInUse and not sqlrdy and msctcp == 0 and not sysRun:
				print('\nTCP in offline mode...')
				time.sleep(.5)

			elif not spcInUse and sysrdy:
				print('\nSPC in Disengage mode...')

			elif spcInUse and sysrdy and not sysRun:
				print('\nSPC in Engage mode...')

			elif spcInUse and msctcp == 301 and sysrdy and sysidle and not sqlrdy:
				print('\nTCP in Standby mode...')

			elif sysrdy and spcInUse and sqlrdy and msctcp == 110 and not sysidle:
				print('\nTCP in Ready mode...')
