...
db_number, start_offset, bit_offset = 233, 0, 0			# SPC_LogQA_RP[DB233]
...
def plcExec(db_number, nGZ, grp_step, fetch_no):
	...
	while True:
		try:
			...
			tt_list.append(TT16)
			TT17 = readInteger(db_number, start_offset[34], bit_offset[0])  # Pipe Pos
			tt_list.append(TT17)
			TT18 = readInteger(db_number, start_offset[36], bit_offset[0])  # PipeDir			tt_list.append(TT18)
			tt_list.append(TT18)

			# Deposit list column content into rows array ----------[]
			if len(tt_list) > 19:
				del tt_list[0:(len(tt_list) - 19)]				# trim columns to shape
