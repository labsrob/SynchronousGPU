...
class tapeTempTabb(ttk.Frame):  # -- Defines the tabbed region for QA param - Tape Temperature --[]
    ...
    def createProcessTT(self):
        ...
        if pRecipe == 1:                   # PLC Data - for Live Production Analysis
            ...
            # SQL Data --------------------#
            self.T1 = 'TTA_' + pWON        # Tape Temperature
            self.T2 = 'TTB_' + pWON
        self.T3 = 'RMA_' + pWON             # Ramp Profile Mapping from SQL table Only
        # ----------------------------------------------------------#
        ...
        # --------- call data block -------------------------------------#
        threading.Thread(target=self.dataControlTT, daemon=True)
...
class substTempTabb(ttk.Frame):
    ...
    def _dataControlST(self):
        ...
        while True:
            ...
            self.canvas.get_tk_widget().after(3, self._stDataPlot)  # Regime = every 10nseconds
            time.sleep(0.2)
