...
def sqlExec(daq, nGZ, grp_step, T1, T2, T3, fetch_no):
    ...
    try:
        ...
        if len(data1) != 0:
            ...
            print('[RP] Halting for 30 sec...')
            time.sleep(30)

    except Exception as e:
        print("[RP] Data trickling on IDX#:", last_t1)  # , e)
        time.sleep(2)

    t1.close()

    # ----------------------------------------------------[From PLC - Tape Temperature]
    try:
        if last_t2 is None:
    ...
    try:
        ...
        print("[ST] Data trickling on IDX#:", last_t3)  # , e)
        time.sleep(2)
