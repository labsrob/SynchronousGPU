	...
	while True:
		try:
			...
			if len(LP_list) > 82:
				...
				print('\nReseLPing Column Size...', len(LP_list))
			...
			if group_step == 1:
				...
				if group_step > 1 and len(arrayLP) >= (nGZ + n2fetch) and fetch_no <= 31:
					...
					print('ReseLPing Rows on Static... Array Size:', len(arrayLP))
					break
				elif group_step > 1 and len(arrayLP) >= (nGZ + n2fetch) and fetch_no >= 31:
					del arrayLP[0:(len(arrayLP) - fetch_no)]
					print('ReseLPing Rows on Move.... Array Size:', len(arrayLP))
					break
