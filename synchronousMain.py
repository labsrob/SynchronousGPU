...
def generate_pdf(rptID, cPipe, psData, custm, usrID, layrN, ring1, ring2, ring3, ring4, SetPt, Value, Stdev, Tvalu, pgiD):
    ...
    for x in range(len(pgiD)):
        ...
        pdf.cell(25, 8, 'mValue', 1, 0, 'C')    # Mean of measured Value
        pdf.cell(25, 8, "cValue", 1, 0, 'C')
...
def get_data(layerN):
    ...
    if sq_con:                               # Enforce connection before processing
        ...
        print('\nAccumulator Length:', szA, szB, szC, szD)
        ...
        for i in range(len(rpData)):
            print('\nTP03-Buffer Length', i, '\t', len(rpData[i]))
            if i == 0:
...
class laserAngleTabb(ttk.Frame):
    ...
    def _dataControlLA(self):
        ...
        # Obtain Volatile Data from PLC Host Server ---------------------------[]
        if not inUseAlready:                        # Load ComsPlc class once
        ...
        while True:
            ...
            if UsePLC_DBS:                                      # Using PLC Data
                ...
                if keyboard.is_pressed("ctrl") and not inProgress:
                    ...
                    self.stDta, self.stDtb = pst.sqlExec(st_con, s_fetch, stp_Sz, s_regm, T1, T2)
                    print("Visualization in Play Mode...")
                print('\nUpdating....')
